...
export function decodeZ64(graphic: ZplGraphic): Uint8Array {
  ...
  if (inflated.length < graphic.totalBytes) throw new Error(`O grÃ¡fico ${graphic.name} estÃ¡ incompleto.`);
...
export function graphicToCanvas(graphic: ZplGraphic): HTMLCanvasElement {
  ...
  if (!context) throw new Error('O navegador nÃ£o disponibilizou o Canvas 2D.');
...
×M:ã†å¡¿y¾yÓ·½ÛÏ8Ó®9÷žøiöôvÛñ½<íý4÷K+süé—÷^r‡^Ï®-