const dictionary: Array<[RegExp, string]> = [
  ...
  [/\bplacas?\s*3d.*p[eÃ©]talas?\b/i, 'PLACAS 3D PÃ‰TALAS'],
  [/\bbuchas?\s+(?:de\s+)?fixa[cÃ§][aÃ£]o\b/i, 'BUCHAS'],
...
export function normalizeProduct(description: string, total = 1): string {
  ...
  if (/\bBUCHINH(?:O|OS)\b/.test(normalized) && /\b60\s*[XÃ—]\s*40\b/.test(normalized)) {
