...
describe('arquivo real da Shopee', () => {
  ...
  it('contém seis páginas gráficas e gera somente três etiquetas', () => {
    const pages = parseZpl(source);
    expect(pages).toHaveLength(6);
    const final = buildFinalZpl([
      { page: pages[0], text: '1 BUCHINHO 60X40', area: { x: 24, y: 580, width: 768, height: 90, template: 'padrão' } },
      { page: pages[2], text: '80 PLACAS 3D PETALAS\n1 CAPIM', area: { x: 24, y: 580, width: 768, height: 90, template: 'padrão' } },
  ...
  it('interpreta o OCR real de múltiplos itens e quantidades internas', () => {
    const text = `
      1 |Placas 3D PVC 50x50 | Renove sua Parede com Pétalas,20 Unidades (R$5,60 Un) 4 K.20.PET
